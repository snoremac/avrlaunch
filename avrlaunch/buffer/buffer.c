...
struct buffer buffer_init(volatile void* data, const uint16_t max_elements, const uint16_t element_size) {
  ...
  memset((void*) data, 0, buffer.max_elements * buffer.element_size);
  return buffer;
}
...
void buffer_reset(struct buffer* buffer) {
  ...
  memset((void*) buffer->data, 0, buffer->max_elements * buffer->element_size);
}
...
volatile void* buffer_shift(struct buffer* buffer) {
  if (buffer->element_count == 0) return NULL;
